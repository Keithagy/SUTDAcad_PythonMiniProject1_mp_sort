// Transcrypt'ed from Python, 2021-07-19 22:52:23
var random = {};
...
export var sortnumber1 = function () {
	var toSort = array.__getslice__ (0, null, 1);
	insertion_sort (toSort);
	var array_str = str (toSort).__getslice__ (1, -(1), 1) + '.';
...
export var sortnumber2 = function () {
	...
	var arr = value.py_split (',');
	var arr_num = [];
	for (var ele of arr) {
		arr_num.append (int (ele));
	}
	insertion_sort (arr_num);
	var array_str = str (arr_num).__getslice__ (1, -(1), 1) + '.';
	document.getElementById ('sorted').innerHTML = array_str;
};
export var insertion_sort = function (array) {
	var n = len (array);
	for (var set = 1; set < n; set++) {
		for (var pos of range (set, 0, -(1))) {
			if (array [pos] < array [pos - 1]) {
				var __left0__ = tuple ([array [pos - 1], array [pos]]);
				array [pos] = __left0__ [0];
				array [pos - 1] = __left0__ [1];
			}
			else {
				break;
			}
